...
(function($, window) {
  ...
  InputLimit = (function() {
    ...
    InputLimit.prototype.extractNumbers = function(val) {
      var matched;
      matched = val.match(this.buildRegex());
    ...
    InputLimit.prototype.process = function() {
      var extr, num, val;
      extr = this.extractNumbers(this.getVal());
      console.log(extr);
      if (extr) {
        num = extr[0];
        val = this.getVal();
        if (((val !== num) && (val !== num + '.')) || (this.occurrances(val) > this.allowedDecimals)) {
